...
import { projectEffectObservableOperationRealization } from "@bang/target-effect";
import { BunRuntime, BunServices } from "@effect/platform-bun";
import { Effect, FileSystem, Path, Schema } from "effect";
import { ChildProcess, ChildProcessSpawner } from "effect/unstable/process";

const fixture = "examples/tiny-bank/account.bang";
const generatedPath = "generated/effect/WithdrawAccountTrace.ts";
const snapshotPath = "tests/snapshots/M008-WithdrawAccountTrace.ts";
...
const program = Effect.scoped(
  Effect.gen(function* () {
    ...
    const source = yield* projectEffectObservableOperationRealization(checked, "WithdrawAccount");
    ...
    if (generated !== snapshot) {
      return yield* new PreparationError({
        message: `generated observable operation kit differs from ${snapshotPath}`,
      });
    }
  }),
);
