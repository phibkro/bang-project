...
The first completed vertical mission is [M000 executable specification spine](design-specs/M000-executable-specification-spine.md). [M001 refined domain value](design-specs/M001-refined-domain-value.md) is active. The [capability dependency map](CAPABILITIES.md) records the current route without treating unexercised architecture as settled.
...
The current demonstrations decode a Core service and a refined `Balance`, reject invalid references and predicates, generate Effect boundaries, compile independent consumers, exercise valid and invalid runtime values, and write scoped evidence manifests under `.bang/evidence/`.
